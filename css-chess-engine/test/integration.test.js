...
describe('Integration — Tactical Tests', () => {
  ...
  it('captures a hanging queen (depth 1)', async () => {
    ...
    const result = await search.findBestMove(chess, { depth: 1 });

    // Knight should capture queen on d4
    assert.ok(result.bestMove, 'Should find a move');
    // The best move should capture the queen - check that queen is captured
    chess.move(result.bestMove);
    // After the best move, black queen on d4 should be gone
  ...
  it('finds mate in 1 (depth 2)', async () => {
    ...
    const chess = new Chess(fen);
    const result = await search.findBestMove(chess, { depth: 2 });
  ...
  it('prefers promotion (depth 2)', async () => {
    ...
    assert.ok(result.bestMove, 'Should find a move');
    // bestMove is now in UCI format (e.g. "a7a8q")
    // Find matching move by UCI notation
